...
const CustomerBrandCard = ({ data, getmovieapi, replce, mainitem, getWishlist, getid,getsingalmalldata }) => {
  ...
  return (
    <>
      <div className="cbc_main_wrapp">
        ...
        {data.is_cinema_product_banner_tiles === 1 ? <>
        </> : <>
          {/* <p className="cbc_price">R{data.price ? data.price : ""} </p> */}
          {data.price ? parseFloat(data.price).toFixed(2).split('.')[0] + '.00' : ""}
        </>}
