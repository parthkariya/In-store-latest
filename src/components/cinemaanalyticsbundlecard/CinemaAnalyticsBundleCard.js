...
import "./CinemaAnalyticsBundleCard.css";
...
const CinemaAnalyticsBundleCard = ({
  item,
  mindx,
  getLeaderboard,
  setTab,
  ...
  const UpdateLeaderboard = async () => {
    ...
    if (weekname1 === "") {
      ...
      const data = await UpdateAnaluticBundleApi(formdata);
      if (data) {
        if (data.success === 1) {
          // ("category-data", data);
          Notification(
            "success",
            "Success!",
            "Analytics Bundle Updated Successfully!"
          );
  ...
  const DeleteLeaderboard = async () => {
    ...
    const data = await deleteAnalyticsBundleApi(formdata);
    if (data) {
      if (data.success === 1) {
        // ("mall-data", data);
        Notification(
          "success",
          "Success!",
          "Analytic Bundle Deleted Successfully!"
        );
  ...
  const Addtocart = async () => {
    ...
    await formdata.append("analytic_bundle_id", item.id);
...
export default CinemaAnalyticsBundleCard;